...
import type { SyntheseVersion } from '../../../shared/types';
...
export default function VersionModal({
  ...
  const { versions, loadVersions, restoreVersion } = useSyntheseStore();

  useEffect(() => {
    if (isOpen) {
      loadVersions(eleveId, domaineId, periodeId, anneeScolaireId);
    }
  }, [isOpen, eleveId, domaineId, periodeId, anneeScolaireId]);

  if (!isOpen) return null;

  const handleRestore = async (version: SyntheseVersion) => {
    const confirmed = window.confirm(
      `Restaurer la version ${version.version} ?\n\nCela creera une nouvelle version avec ce texte.`
    );
    if (!confirmed) return;

    try {
      await restoreVersion(eleveId, domaineId, periodeId, anneeScolaireId, version.id);
      onClose();
    } catch {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">{domaineNom}</p>
          </div>
          <button
            onClick={onClose}
